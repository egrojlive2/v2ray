...
class V2ray:
    ...
    def update():
        if is_ipv6(get_ip()):
            ...
            print(ColorStr.fuchsia(_("download v2ray-linux-xx.zip and run 'bash <(curl -L -s https://raw.githubusercontent.com/egrojlive/v2ray/main/go.sh) -l v2ray-linux-xx.zip' to update")))
            return
        if os.path.exists("/.dockerenv"):
            V2ray.stop()
        subprocess.Popen("curl -L -s https://raw.githubusercontent.com/egrojlive/v2ray/main/go.sh|bash", shell=True).wait()
        if os.path.exists("/.dockerenv"):
            V2ray.start()
    ...
    def check(cls):
        ...
        if not os.path.exists("/usr/bin/v2ray/v2ray"):
            ...
            if is_ipv6(get_ip()):
                subprocess.Popen("curl -Ls https://raw.githubusercontent.com/egrojlive/v2ray/main/go.sh -o temp.sh", shell=True).wait()
